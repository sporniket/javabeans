...
import static com.sporniket.libre.javabeans.seeg.CodeGeneratorHelper.FROM_DEF_CLASS_TO_ID_CLASS;
			...
			throws Exception
	{
		...
		try (Connection con = getConnection(configDatabase))
		{
			...
			workspace.getClasses().forEach(c -> {
				FROM_DEF_CLASS_TO_ENTITY.generate(c, targetDir, targetPackage, out);
				if (c.pkeysColumns.size() > 1)
				{
					FROM_DEF_CLASS_TO_ID_CLASS.generate(c, targetDir, targetPackage, out);
				}
			});
